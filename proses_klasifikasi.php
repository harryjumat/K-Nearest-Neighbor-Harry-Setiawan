...
// error_reporting(0);
// ====PROSES CASE FOLDING====
// ambil data tweet dari tabel documents
...
class Filtering
{
	...
	public function getToken($token, $q_kode, $nbrwords2 = 5)
	{
		...
		//membandingkan nilai dari dua (atau lebih) array dan mengembalikan perbedaannya.
...
while ($row_uji = mysqli_fetch_array($query_uji)) {
	...
	$tweet = mysqli_query($koneksi, "SELECT doc_id FROM documents") or die(mysqli_error($koneksi));
	$num_rows = mysqli_num_rows($tweet);
	...
	// PROSES PERNAMBAHAN DATA AKHIR TWEET
